...
async def test_delayed_old_wait_never_reaches_telegram():
    ...
    with db_session() as db:
        row = db.execute(select(TelegramNotification).where(
            TelegramNotification.event_id == event_id)).scalar_one_or_none()
        # Ordinary WAIT is audit/log-only and never reaches the delivery
        # outbox, so a later decision cannot accidentally deliver it.
        assert row is None
