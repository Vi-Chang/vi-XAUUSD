...
class AnalysisRun(Base):
    ...
    grading_version: Mapped[str] = mapped_column(String(32), default="signal-score-v1")
    trade_status: Mapped[str] = mapped_column(String(32), default="WAIT_CONFIRMATION")
    can_enter: Mapped[bool] = mapped_column(Boolean, default=False)
...
class Position(Base):
    ...
    position_timeframe: Mapped[str] = mapped_column(String(8), default="unknown")
    original_thesis: Mapped[str] = mapped_column(String(500), default="")
    max_loss_usd: Mapped[float | None] = mapped_column(Float, nullable=True)
    allow_event_hold: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    open_time: Mapped[datetime] = mapped_column(DateTime(timezone=True))
